...
function ciniki_info_contentAdd(&$ciniki) {
    ...
    $rc = ciniki_info_checkAccess($ciniki, $args['tnid'], 'ciniki.info.contentAdd'); 
    if( $rc['stat'] != 'ok' ) { 
        return $rc;
    } 
    ...
    if( $rc['stat'] != 'ok' ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.info.5', 'msg'=>'Unable to get a new UUID', 'err'=>$rc['err']));
    }
    $args['uuid'] = $rc['uuid'];
    ...
    if( $rc['num_rows'] > 0 ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.info.6', 'msg'=>'You already have content with this name, you must choose another.'));
    }
    ...
    return ciniki_core_objectAdd($ciniki, $args['tnid'], 'ciniki.info.content', $args, 0x07);
}
?>
