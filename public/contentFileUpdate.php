...
// <rsp stat='ok' />
//
function ciniki_info_contentFileUpdate(&$ciniki) {
    //  
    ...
    $rc = ciniki_info_checkAccess($ciniki, $args['business_id'], 'ciniki.info.contentFileUpdate'); 
    if( $rc['stat'] != 'ok' ) { 
        return $rc;
    }
    ...
    //
    $strsql = "SELECT id, content_id, name, permalink FROM ciniki_info_content_files "
    ...
    if( !isset($rc['file']) ) {
        return array('stat'=>'fail', 'err'=>array('pkg'=>'ciniki', 'code'=>'1669', 'msg'=>'File does not exist.'));
    }
    $file = $rc['file'];
    ...
    if( isset($args['permalink']) ) {
        $strsql = "SELECT id, name, permalink FROM ciniki_info_content_files "
            ...
            . "AND id <> '" . ciniki_core_dbQuote($ciniki, $args['file_id']) . "' "
            . "";
        $rc = ciniki_core_dbHashQuery($ciniki, $strsql, 'ciniki.info', 'files');
        if( $rc['stat'] != 'ok' ) {
            return $rc;
        }
        if( $rc['num_rows'] > 0 ) {
            return array('stat'=>'fail', 'err'=>array('pkg'=>'ciniki', 'code'=>'1670', 'msg'=>'You already have a file with this name, please choose another name.'));
        }
    }
    ...
    return ciniki_core_objectUpdate($ciniki, $args['business_id'], 'ciniki.info.content_file', $args['file_id'], $args, 0x07);
}
?>
