...
function ciniki_info_contentImageUpdate(&$ciniki) {
    ...
    $rc = ciniki_info_checkAccess($ciniki, $args['tnid'], 'ciniki.info.contentImageUpdate'); 
    if( $rc['stat'] != 'ok' ) { 
        return $rc;
    }
    ...
    if( !isset($rc['item']) ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.info.25', 'msg'=>'Image not found'));
    }
    $item = $rc['item'];
    ...
    if( isset($args['name']) ) {
        ...
        if( $rc['num_rows'] > 0 ) {
            return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.info.26', 'msg'=>'You already have an image with this name, please choose another name'));
        }
    }
    ...
    return ciniki_core_objectUpdate($ciniki, $args['tnid'], 'ciniki.info.content_image', $args['content_image_id'], $args);
}
?>
