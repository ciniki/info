//
// The info app to manage committees information
//
function ciniki_info_committees() {
    this.content_type = 27;
    this.init = function() {
        //
        // The panel to display the add form
        //
        this.edit = new M.panel('Committees',
            'ciniki_info_committees', 'edit',
            'mc', 'medium mediumaside', 'sectioned', 'ciniki.info.committees.edit');
        this.edit.data = {};    
...
//              'primary_image_url':{'label':'URL', 'type':'text'},
            ...
            'children':{'label':'Committees', 'type':'simplegrid', 'num_cols':1, 
                'addTxt':'Add Committee',
                'addFn':'M.ciniki_info_committees.showChildEdit(\'M.ciniki_info_committees.updateChildren();\',M.ciniki_info_committees.edit.content_id,0);',
                },
            '_buttons':{'label':'', 'buttons':{
                'save':{'label':'Save', 'fn':'M.ciniki_info_committees.saveContent();'},
            }},
        };
        ...
        this.edit.addDropImage = function(iid) {
            M.ciniki_info_committees.edit.setFieldValue('primary_image_id', iid, null, null);
            return true;
        };
        ...
        this.edit.rowFn = function(s, i, d) {
            if( s == 'children' ) {
                return 'M.ciniki_info_committees.showChildEdit(\'M.ciniki_info_committees.updateChildren();\',M.ciniki_info_committees.edit.content_id,\'' + d.child.id + '\');';
            }
        };
        this.edit.addButton('save', 'Save', 'M.ciniki_info_committees.saveContent();');
        this.edit.addClose('Cancel');

        //
        // The child edit panel 
        //
        this.childedit = new M.panel('Committees',
            'ciniki_info_committees', 'childedit',
            'mc', 'medium mediumaside', 'sectioned', 'ciniki.info.committees.childedit');
        this.childedit.data = {};   
...
//              'primary_image_url':{'label':'URL', 'type':'text'},
            ...
            '_content':{'label':'Description', 'fields':{
                'content':{'label':'', 'type':'textarea', 'size':'medium', 'hidelabel':'yes'},
            }},
            '_buttons':{'label':'', 'buttons':{
                'save':{'label':'Save', 'fn':'M.ciniki_info_committees.saveChildContent();'},
                'delete':{'label':'Delete', 'fn':'M.ciniki_info_committees.deleteChild();'},
            }},
        };
        ...
        this.childedit.addDropImage = function(iid) {
            M.ciniki_info_committees.childedit.setFieldValue('primary_image_id', iid, null, null);
            return true;
        };
        ...
        this.childedit.addButton('save', 'Save', 'M.ciniki_info_committees.saveChildContent();');
    ...
    this.start = function(cb, appPrefix, aG) {
        ...
        // Create container
        //
        var appContainer = M.createContainer(appPrefix, 'ciniki_info_committees', 'yes');
    ...
    this.showEdit = function(cb) {
        M.api.getJSONCb('ciniki.info.contentGet', {'tnid':M.curTenantID,
            'content_type':this.content_type, 'children':'list'}, function(rsp) {
                ...
                var p = M.ciniki_info_committees.edit;
                p.data = rsp.content;
        ...
        if( c != null ) {
            M.api.postJSONFormData('ciniki.info.contentUpdate', 
                {'tnid':M.curTenantID, 'content_id':this.edit.content_id}, c, function(rsp) {
                    ...
                    M.ciniki_info_committees.edit.close();
                });
        } else {
            this.edit.close();
        }
    ...
    this.updateChildren = function() {
        M.api.getJSONCb('ciniki.info.contentGet', {'tnid':M.curTenantID,
            'content_id':M.ciniki_info_committees.edit.content_id, 'children':'list'}, function(rsp) {
                if( rsp.stat != 'ok' ) {
                    M.api.err(rsp);
                    return false;
                }
        ...
        if( this.childedit.content_id > 0 ) {
            M.api.getJSONCb('ciniki.info.contentGet', {'tnid':M.curTenantID,
                'content_id':this.childedit.content_id}, function(rsp) {
                    ...
                    var p = M.ciniki_info_committees.childedit;
                    p.data = rsp.content;
                    p.refresh();
                    p.show(cb);
                });
            ...
            if( c != null ) {
                M.api.postJSONFormData('ciniki.info.contentUpdate', 
                    {'tnid':M.curTenantID, 'content_id':this.childedit.content_id}, c, function(rsp) {
                        ...
                        M.ciniki_info_committees.childedit.close();
                    });
            } else {
    ...
    this.deleteChild = function() {
        M.confirm('Are you sure you want to delete this committee?',null,function() {
            var rsp = M.api.getJSONCb('ciniki.info.contentDelete', {'tnid':M.curTenantID, 
                'content_id':M.ciniki_info_committees.childedit.content_id}, function(rsp) {
                    if( rsp.stat != 'ok' ) {
                        M.api.err(rsp);
                        return false;
                    }
