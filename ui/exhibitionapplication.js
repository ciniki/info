//
// The info app to manage exhibition application information
//
function ciniki_info_exhibitionapplication() {
    this.content_type = 10;
    this.init = function() {
        //
        // The panel to display the add form
        //
        this.edit = new M.panel('Application Details',
            'ciniki_info_exhibitionapplication', 'edit',
            'mc', 'medium mediumaside', 'sectioned', 'ciniki.info.exhibitionapplication.edit');
        this.edit.data = {};    
...
//              'primary_image_url':{'label':'URL', 'type':'text'},
            ...
            '_content':{'label':'Application Details', 'fields':{
            ...
            'files':{'label':'Application Forms',
                ...
                'addFn':'M.ciniki_info_exhibitionapplication.showFileEdit(\'M.ciniki_info_exhibitionapplication.updateFiles();\',M.ciniki_info_exhibitionapplication.edit.content_id,0);',
                },
            '_buttons':{'label':'', 'buttons':{
                'save':{'label':'Save', 'fn':'M.ciniki_info_exhibitionapplication.saveContent();'},
            }},
        };
        ...
        this.edit.addDropImage = function(iid) {
            M.ciniki_info_exhibitionapplication.edit.setFieldValue('primary_image_id', iid, null, null);
            return true;
        };
        ...
        this.edit.rowFn = function(s, i, d) {
            return 'M.ciniki_info_exhibitionapplication.showFileEdit(\'M.ciniki_info_exhibitionapplication.updateFiles();\',M.ciniki_info_exhibitionapplication.edit.content_id,\'' + d.file.id + '\');';
        };
        this.edit.addButton('save', 'Save', 'M.ciniki_info_exhibitionapplication.saveContent();');
        ...
        this.addfile = new M.panel('Add File',
            'ciniki_info_exhibitionapplication', 'addfile',
            'mc', 'medium', 'sectioned', 'ciniki.info.exhibitionapplication.editfile');
...
//          }},
            '_save':{'label':'', 'buttons':{
                'save':{'label':'Save', 'fn':'M.ciniki_info_exhibitionapplication.addFile();'},
            }},
        };
        ...
        this.addfile.addButton('save', 'Save', 'M.ciniki_info_exhibitionapplication.addFile();');
        ...
        this.editfile = new M.panel('File',
            'ciniki_info_exhibitionapplication', 'editfile',
            'mc', 'medium', 'sectioned', 'ciniki.info.exhibitionapplication.editfile');
        ...
        this.editfile.sections = {
            ...
            '_save':{'label':'', 'buttons':{
                'save':{'label':'Save', 'fn':'M.ciniki_info_exhibitionapplication.saveFile();'},
                'download':{'label':'Download', 'fn':'M.ciniki_info_exhibitionapplication.downloadFile(M.ciniki_info_exhibitionapplication.editfile.file_id);'},
                'delete':{'label':'Delete', 'fn':'M.ciniki_info_exhibitionapplication.deleteFile();'},
            }},
        };
        ...
        this.editfile.addButton('save', 'Save', 'M.ciniki_info_exhibitionapplication.saveFile();');
    ...
    this.start = function(cb, appPrefix, aG) {
        ...
        // Create container
        //
        var appContainer = M.createContainer(appPrefix, 'ciniki_info_exhibitionapplication', 'yes');
    ...
    this.showEdit = function(cb) {
        M.api.getJSONCb('ciniki.info.contentGet', {'tnid':M.curTenantID,
            'content_type':M.ciniki_info_exhibitionapplication.content_type, 'files':'yes'}, function(rsp) {
                if( rsp.stat != 'ok' ) {
                    M.api.err(rsp);
                    return false;
                }
                var p = M.ciniki_info_exhibitionapplication.edit;
                p.data = rsp.content;
        ...
        if( c != null ) {
            M.api.postJSONFormData('ciniki.info.contentUpdate', 
                {'tnid':M.curTenantID, 'content_id':this.edit.content_id}, c, function(rsp) {
                    ...
                    M.ciniki_info_exhibitionapplication.edit.close();
                });
        } else {
            M.ciniki_info_exhibitionapplication.edit.close();
        }
        ...
        if( fid != null && fid > 0 ) {
            ...
            var rsp = M.api.getJSONCb('ciniki.info.contentFileGet', 
                {'tnid':M.curTenantID, 'file_id':this.editfile.file_id}, function(rsp) {
                    ...
                    var p = M.ciniki_info_exhibitionapplication.editfile;
                    p.data = rsp.file;
                    p.refresh();
                    p.show(cb);
                });
    ...
    this.updateFiles = function() {
        M.api.getJSONCb('ciniki.info.contentGet', {'tnid':M.curTenantID,
            'content_id':M.ciniki_info_exhibitionapplication.editfile.content_id, 'files':'yes'}, function(rsp) {
                if( rsp.stat != 'ok' ) {
                    M.api.err(rsp);
                    return false;
                }
    ...
    this.addFile = function() {
        ...
        M.api.postJSONFormData('ciniki.info.contentFileAdd', 
            {'tnid':M.curTenantID, 
            'content_id':M.ciniki_info_exhibitionapplication.addfile.content_id}, c, function(rsp) {
                if( rsp.stat != 'ok' ) {
                    M.api.err(rsp);
                    return false;
                } 
                M.ciniki_info_exhibitionapplication.addfile.close();
            });
    };
            ...
            M.api.postJSONFormData('ciniki.info.contentFileUpdate', 
                {'tnid':M.curTenantID, 'file_id':this.editfile.file_id}, c,
                    function(rsp) {
                        ...
                        M.ciniki_info_exhibitionapplication.editfile.close();
                    });
        } else {
            this.editfile.close();
        }
    ...
    this.deleteFile = function() {
        M.confirm('Are you sure you want to delete \'' + this.editfile.data.name + '\'?  All information about the file will be removed and unrecoverable.',null,function() {
            M.api.getJSONCb('ciniki.info.contentFileDelete', {'tnid':M.curTenantID, 
                'file_id':M.ciniki_info_exhibitionapplication.editfile.file_id}, function(rsp) {
                    if( rsp.stat != 'ok' ) {
                        M.api.err(rsp);
                        return false;
                    } 
