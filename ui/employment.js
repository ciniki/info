//
// The info app to manage employment information
//
function ciniki_info_employment() {
	this.content_type = 21;
	this.init = function() {
		//
		// The panel to display the add form
		//
		this.edit = new M.panel('Employment',
			'ciniki_info_employment', 'edit',
			'mc', 'medium mediumaside', 'sectioned', 'ciniki.info.employment.edit');
		this.edit.data = {};	
		this.edit.content_id = 0;
		this.edit.sections = {
			'_image':{'label':'', 'aside':'yes', 'type':'imageform', 'fields':{
				'primary_image_id':{'label':'', 'type':'image_id', 'hidelabel':'yes', 
					'controls':'all', 'employment':'no'},
			}},
			'_image_caption':{'label':'', 'aside':'yes', 'fields':{
				'primary_image_caption':{'label':'Caption', 'type':'text'},
//				'primary_image_url':{'label':'URL', 'type':'text'},
			}},
			'_content':{'label':'Employment', 'fields':{
				'content':{'label':'', 'type':'textarea', 'size':'large', 'hidelabel':'yes'},
			}},
			'_buttons':{'label':'', 'buttons':{
				'save':{'label':'Save', 'fn':'M.ciniki_info_employment.saveContent();'},
			}},
		};
		...
		this.edit.addDropImage = function(iid) {
			M.ciniki_info_employment.edit.setFieldValue('primary_image_id', iid, null, null);
			return true;
		};
		...
		this.edit.rowFn = function(s, i, d) {
			return 'M.ciniki_info_employment.showFileEdit(\'M.ciniki_info_employment.updateFiles();\',M.ciniki_info_employment.edit.content_id,\'' + d.file.id + '\');';
		};
		this.edit.addButton('save', 'Save', 'M.ciniki_info_employment.saveContent();');
	...
	this.showEdit = function(cb) {
		M.api.getJSONCb('ciniki.info.contentGet', {'business_id':M.curBusinessID,
			'content_type':this.content_type}, function(rsp) {
				...
				var p = M.ciniki_info_employment.edit;
				p.data = rsp.content;
		...
		if( c != null ) {
			M.api.postJSONFormData('ciniki.info.contentUpdate', 
				{'business_id':M.curBusinessID, 'content_id':this.edit.content_id}, c, function(rsp) {
					...
					M.ciniki_info_employment.edit.close();
				});
		} else {
			this.edit.close();
		}
