//
// The info app to manage financialassistance information
//
function ciniki_info_financialassistance() {
	this.content_type = 19;
	this.init = function() {
		//
		// The panel to display the add form
		//
		this.edit = new M.panel('Financial Assistance',
			'ciniki_info_financialassistance', 'edit',
			'mc', 'medium mediumaside', 'sectioned', 'ciniki.info.financialassistance.edit');
		this.edit.data = {};	
		this.edit.content_id = 0;
		this.edit.sections = {
			'_image':{'label':'', 'aside':'yes', 'type':'imageform', 'fields':{
				'primary_image_id':{'label':'', 'type':'image_id', 'hidelabel':'yes', 
					'controls':'all', 'financialassistance':'no'},
			}},
			'_image_caption':{'label':'', 'aside':'yes', 'fields':{
				'primary_image_caption':{'label':'Caption', 'type':'text'},
//				'primary_image_url':{'label':'URL', 'type':'text'},
			}},
			'_content':{'label':'Financial Assistance', 'fields':{
				'content':{'label':'', 'type':'textarea', 'size':'large', 'hidelabel':'yes'},
			}},
			'_buttons':{'label':'', 'buttons':{
				'save':{'label':'Save', 'fn':'M.ciniki_info_financialassistance.saveContent();'},
			}},
		};
		...
		this.edit.addDropImage = function(iid) {
			M.ciniki_info_financialassistance.edit.setFieldValue('primary_image_id', iid, null, null);
			return true;
		};
		...
		this.edit.rowFn = function(s, i, d) {
			return 'M.ciniki_info_financialassistance.showFileEdit(\'M.ciniki_info_financialassistance.updateFiles();\',M.ciniki_info_financialassistance.edit.content_id,\'' + d.file.id + '\');';
		};
		this.edit.addButton('save', 'Save', 'M.ciniki_info_financialassistance.saveContent();');
	...
	this.showEdit = function(cb) {
		M.api.getJSONCb('ciniki.info.contentGet', {'business_id':M.curBusinessID,
			'content_type':this.content_type}, function(rsp) {
				...
				var p = M.ciniki_info_financialassistance.edit;
				p.data = rsp.content;
		...
		if( c != null ) {
			M.api.postJSONFormData('ciniki.info.contentUpdate', 
				{'business_id':M.curBusinessID, 'content_id':this.edit.content_id}, c, function(rsp) {
					...
					M.ciniki_info_financialassistance.edit.close();
				});
		} else {
			this.edit.close();
		}
