//
// The info app to manage Warranty information
//
function ciniki_info_warranty() {
	this.content_type = 11;
	this.init = function() {
		//
		// The panel to display the add form
		//
		this.edit = new M.panel('Warranty',
			'ciniki_info_warranty', 'edit',
			'mc', 'medium mediumaside', 'sectioned', 'ciniki.info.warranty.edit');
		this.edit.data = {};	
...
//				'primary_image_url':{'label':'URL', 'type':'text'},
			...
			'_content':{'label':'Warranty', 'fields':{
				'content':{'label':'', 'type':'textarea', 'size':'large', 'hidelabel':'yes'},
			}},
			'_buttons':{'label':'', 'buttons':{
				'save':{'label':'Save', 'fn':'M.ciniki_info_warranty.saveContent();'},
			}},
		};
		...
		this.edit.addDropImage = function(iid) {
			M.ciniki_info_warranty.edit.setFieldValue('primary_image_id', iid, null, null);
			return true;
		};
		...
		this.edit.rowFn = function(s, i, d) {
			return 'M.ciniki_info_warranty.showFileEdit(\'M.ciniki_info_warranty.updateFiles();\',M.ciniki_info_warranty.edit.content_id,\'' + d.file.id + '\');';
		};
		this.edit.addButton('save', 'Save', 'M.ciniki_info_warranty.saveContent();');
	...
	this.start = function(cb, appPrefix, aG) {
		...
		// Create container
		//
		var appContainer = M.createContainer(appPrefix, 'ciniki_info_warranty', 'yes');
	...
	this.showEdit = function(cb) {
		M.api.getJSONCb('ciniki.info.contentGet', {'business_id':M.curBusinessID,
			'content_type':this.content_type}, function(rsp) {
				...
				var p = M.ciniki_info_warranty.edit;
				p.data = rsp.content;
		...
		if( c != null ) {
			M.api.postJSONFormData('ciniki.info.contentUpdate', 
				{'business_id':M.curBusinessID, 'content_id':this.edit.content_id}, c, function(rsp) {
					...
					M.ciniki_info_warranty.edit.close();
				});
		} else {
			this.edit.close();
		}
