//
// The info app to manage Artist Statement information
//
function ciniki_info_artiststatement() {
    this.content_type = 2;
    this.init = function() {
        //
        // The panel to display the add form
        //
        this.edit = new M.panel('Artist Statement',
            'ciniki_info_artiststatement', 'edit',
            'mc', 'medium mediumaside', 'sectioned', 'ciniki.info.artiststatement.edit');
        this.edit.data = {};    
...
//              'primary_image_url':{'label':'URL', 'type':'text'},
            ...
            '_content':{'label':'Artist Statement', 'fields':{
                'content':{'label':'', 'type':'textarea', 'size':'large', 'hidelabel':'yes'},
            }},
            '_buttons':{'label':'', 'buttons':{
                'save':{'label':'Save', 'fn':'M.ciniki_info_artiststatement.saveContent();'},
            }},
        };
        ...
        this.edit.addDropImage = function(iid) {
            M.ciniki_info_artiststatement.edit.setFieldValue('primary_image_id', iid, null, null);
            return true;
        };
        ...
        this.edit.rowFn = function(s, i, d) {
            return 'M.ciniki_info_artiststatement.showFileEdit(\'M.ciniki_info_artiststatement.updateFiles();\',M.ciniki_info_artiststatement.edit.content_id,\'' + d.file.id + '\');';
        };
        this.edit.addButton('save', 'Save', 'M.ciniki_info_artiststatement.saveContent();');
    ...
    this.start = function(cb, appPrefix, aG) {
        ...
        // Create container
        //
        var appContainer = M.createContainer(appPrefix, 'ciniki_info_artiststatement', 'yes');
    ...
    this.showEdit = function(cb) {
        M.api.getJSONCb('ciniki.info.contentGet', {'tnid':M.curTenantID,
            'content_type':this.content_type}, function(rsp) {
                ...
                var p = M.ciniki_info_artiststatement.edit;
                p.data = rsp.content;
        ...
        if( c != null ) {
            M.api.postJSONFormData('ciniki.info.contentUpdate', 
                {'tnid':M.curTenantID, 'content_id':this.edit.content_id}, c, function(rsp) {
                    ...
                    M.ciniki_info_artiststatement.edit.close();
                });
        } else {
            this.edit.close();
        }
