//
// The info app to manage staff information
//
function ciniki_info_staff() {
    this.content_type = 22;
    this.init = function() {
        //
        // The panel to display the add form
        //
        this.edit = new M.panel('Staff',
            'ciniki_info_staff', 'edit',
            'mc', 'medium mediumaside', 'sectioned', 'ciniki.info.staff.edit');
        this.edit.data = {};    
...
//              'primary_image_url':{'label':'URL', 'type':'text'},
            ...
            'children':{'label':'Staff', 'type':'simplegrid', 'num_cols':1, 
                'addTxt':'Add Staff',
                'addFn':'M.ciniki_info_staff.showChildEdit(\'M.ciniki_info_staff.updateChildren();\',M.ciniki_info_staff.edit.content_id,0);',
                },
            '_buttons':{'label':'', 'buttons':{
                'save':{'label':'Save', 'fn':'M.ciniki_info_staff.saveContent();'},
            }},
        };
        ...
        this.edit.addDropImage = function(iid) {
            M.ciniki_info_staff.edit.setFieldValue('primary_image_id', iid, null, null);
            return true;
        };
        ...
        this.edit.rowFn = function(s, i, d) {
            if( s == 'children' ) {
                return 'M.ciniki_info_staff.showChildEdit(\'M.ciniki_info_staff.updateChildren();\',M.ciniki_info_staff.edit.content_id,\'' + d.child.id + '\');';
            }
        };
        this.edit.addButton('save', 'Save', 'M.ciniki_info_staff.saveContent();');
        this.edit.addClose('Cancel');

        //
        // The child edit panel 
        //
        this.childedit = new M.panel('Staff',
            'ciniki_info_staff', 'childedit',
            'mc', 'medium mediumaside', 'sectioned', 'ciniki.info.staff.childedit');
        this.childedit.data = {};   
...
//              'primary_image_url':{'label':'URL', 'type':'text'},
            ...
            '_buttons':{'label':'', 'buttons':{
                'save':{'label':'Save', 'fn':'M.ciniki_info_staff.saveChildContent();'},
                'delete':{'label':'Delete', 'fn':'M.ciniki_info_staff.deleteChild();'},
            }},
        };
        ...
        this.childedit.addDropImage = function(iid) {
            M.ciniki_info_staff.childedit.setFieldValue('primary_image_id', iid, null, null);
            return true;
        };
        ...
        this.childedit.addButton('save', 'Save', 'M.ciniki_info_staff.saveChildContent();');
    ...
    this.showEdit = function(cb) {
        M.api.getJSONCb('ciniki.info.contentGet', {'business_id':M.curBusinessID,
            'content_type':this.content_type, 'children':'list'}, function(rsp) {
                ...
                var p = M.ciniki_info_staff.edit;
                p.data = rsp.content;
        ...
        if( c != null ) {
            M.api.postJSONFormData('ciniki.info.contentUpdate', 
                {'business_id':M.curBusinessID, 'content_id':this.edit.content_id}, c, function(rsp) {
                    ...
                    M.ciniki_info_staff.edit.close();
                });
        } else {
            this.edit.close();
        }
    ...
    this.updateChildren = function() {
        M.api.getJSONCb('ciniki.info.contentGet', {'business_id':M.curBusinessID,
            'content_id':M.ciniki_info_staff.edit.content_id, 'children':'list'}, function(rsp) {
                if( rsp.stat != 'ok' ) {
                    M.api.err(rsp);
                    return false;
                }
        ...
        if( this.childedit.content_id > 0 ) {
            M.api.getJSONCb('ciniki.info.contentGet', {'business_id':M.curBusinessID,
                'content_id':this.childedit.content_id}, function(rsp) {
                    ...
                    var p = M.ciniki_info_staff.childedit;
                    p.data = rsp.content;
                    p.refresh();
                    p.show(cb);
                });
